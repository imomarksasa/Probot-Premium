...
client.on("message", message => {
	...
	if(!message.member.hasPermission('MANAGE_ROLES')) return message.channel.send(' **__ You do not have permissions___**');
	if( msg.toLowerCase().startsWith( prefix + '-role' ) ){
		if( !args[0] ) return message.reply( '**:x: Please place the person to withdraw from**' );
		if( !args[1] ) return message.reply( '**:x: Please place the rank to be withdrawn from the person**' );
		var role = msg.split(' ').slice(2).join(" ").toLowerCase(); 
		var role1 = message.guild.roles.filter( r=>r.name.toLowerCase().indexOf(role)>-1 ).first(); 
		if( !role1 ) return message.reply( '**:x: Please specify the rank to be withdrawn from**' );if( message.mentions.members.first() ){
			message.mentions.members.first().removeRole( role1 );
			return message.reply('**:white_check_mark: [ '+role1.name+' ] Rank [ '+args[0]+' ] Pulled from **');
		}
		if( args[0].toLowerCase() == "all" ){
			message.guild.members.forEach(m=>m.removeRole( role1 ))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] All rank was withdrawn**');
		} else if( args[0].toLowerCase() == "bots" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] M pull of the bots rank**');
		} else if( args[0].toLowerCase() == "humans" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.removeRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] M pull of the mortal rank**');
		} 	
	} else {
		if( !args[0] ) return message.reply( '**:x: Please put the person to be given the rank**' );
		if( !args[1] ) return message.reply( '**:x: Please specify the grade to be given to the person**' );
		var role = msg.split(' ').slice(2).join(" ").toLowerCase(); 
		var role1 = message.guild.roles.filter( r=>r.name.toLowerCase().indexOf(role)>-1 ).first(); 
		if( !role1 ) return message.reply( '**:x: Please specify the grade to be given to the person**' );if( message.mentions.members.first() ){
			message.mentions.members.first().addRole( role1 );
			return message.reply('**:white_check_mark: [ '+role1.name+' ] Rank [ '+args[0]+' ] tender **');
		}
		if( args[0].toLowerCase() == "all" ){
			message.guild.members.forEach(m=>m.addRole( role1 ))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] All rank was given**');
		} else if( args[0].toLowerCase() == "bots" ){
			message.guild.members.filter(m=>m.user.bot).forEach(m=>m.addRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] Bots were given rank**');
		} else if( args[0].toLowerCase() == "humans" ){
			message.guild.members.filter(m=>!m.user.bot).forEach(m=>m.addRole(role1))
			return	message.reply('**:white_check_mark: [ '+role1.name+' ] Humans were given rank**');
		} 
	} 
});
  ...
  client.on('message', async message => {
    ...
    .setImage('https://c.top4top.net/p_1029o1te41.png')
      ...
      if(message.content.startsWith(prefix + "tempmute")) {
          if(!message.member.hasPermission('MUTE_MEMBERS')) return message.channel.send('**Unfortunately you do not have permission** `MUTE_MEMBERS`' );
          if(!mutePerson) return message.channel.sendEmbed(muteembed)
          if(mutePerson === message.author) return message.channel.send('**-In a way you give yourself death**');
          if(mutePerson === client.user) return message.channel.send('**-I can not give a bot:)**');
          if(message.guild.member(mutePerson).roles.has(muteRole.id)) return message.channel.send('**-This person is already mated**');
          if(!muteRole) return message.guild.createRole({ name: "Muted", permissions: [] });
          if(!time) return message.channel.send("**-Time Books**");
...
client.on('message',message =>{
  var command = message.content.toLowerCase().split(" ")[0];
    ...
    if(command == prefix + 'unban') {
        ...
        if(!args[1]) return  message.channel.send(':information_source:  `#kick <@id>`You must select someone');
...
client.on('message', message => {
  var prefix = "#";
    ...
    if (message.mentions.users.size < 1) return message.reply(":information_source: `#kick @OrochiX` You must select someone");
...
client.on('message', message => {
     ...
     if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply(' **__ You do not have permissions__**');
                message.channel.overwritePermissions(message.guild.id, {
              SEND_MESSAGES: false
  
                }).then(() => {
                    message.reply("**__ You have closed chat__ :white_check_mark: **")
                });
                  }
     ...
     if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('**__ You do not have permissions__**');
              ...
              SEND_MESSAGES: true
                    ...
                    message.reply("**__ Chats is open__:white_check_mark:**")
                });
      }
...
client.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'setStreaming')) {
      if (!devs.includes(message.author.id)) return message.channel.send("<@529646570983915551 > only this guy can do restart the bot so don't try again :wink:.");
      message.delete();
